...
- Added the standalone `hindsight-agent-bridge` release assets for Amp, Codex, and Grok, with shared OMP memory scoping, lifecycle hooks, MCP tools, embedded skill policy, and project observation scopes.
### Fixed

- Fixed signed Hindsight bridge releases failing the pre-notarization smoke check; release binaries now verify their version and project scope before publication.
